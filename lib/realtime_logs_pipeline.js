...
const { KinesisEventSource,  SqsEventSource } = require('@aws-cdk/aws-lambda-event-sources');
...
class RealTimeLogsPipeline extends Stack {
  constructor(scope, id, props) {
    ...
    // Create a new Kinesis Data stream to steam real-time logs data
    ...
    // Canopy Database name and real-time logs table name
    ...
    const KinesisEventSourceConfig = new KinesisEventSource(stream, {
      ...
      // this needs to be dynamically adjusted based on user input
      maxBatchingWindow: Duration.seconds(0),           // The maximum amount of time to gather records before invoking the function.
      maxRecordAge: Duration.minutes(5),                // The maximum age of a record that Lambda sends to a function for processing.
    ...
    const SqsEventSourceConfig = new SqsEventSource(deadLetterQueue, {
      ...
      maxBatchingWindow: Duration.seconds(1),     // The maximum amount of time to gather records before invoking the function.
      reportBatchItemFailures: true,              // Allow functions to return partially successful responses for a batch of records.
    });
