...
const { CfnOutput } =  require('aws-cdk-lib/core');
...
class EC2_Backend_Deployment extends cdk.Stack {
  constructor(scope, id, props) {
    ...
    // Expose the EC2 instance public IP to other CDK stacks
    new CfnOutput(this, 'EC2InstancePublicIP', {
      value: instance.instancePublicIp,
      exportName: 'CanopyBackendIP',
    });
    ...
    # grant "ec2-user" access to the docker daemon in order to run docker commands
    usermod -a -G docker ec2-user

    # Install Docker Compose
    sudo curl -L "https://github.com/docker/compose/releases/download/1.29.2/docker-compose-$(uname -s)-$(uname -m)" -o /usr/local/bin/docker-compose
    
    # add execute permission to the docker-compose file
    sudo chmod +x /usr/local/bin/docker-compose

    # Create directory for Canopy's backend deployment
    sudo mkdir ./home/ec2-user/canopy-backend

    # Copy YAML file from deployment package
    aws s3 cp ${dockerComposeAsset.s3ObjectUrl} ./home/ec2-user/canopy-backend

    cd ./home/ec2-user/canopy-backend
    
    # rename the imported YAML file from "<random hash value>.yml" to docker-compose.yml
