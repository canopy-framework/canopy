...
const { CloudFrontClient, CreateRealtimeLogConfigCommand, GetDistributionConfigCommand, UpdateDistributionCommand } = require('@aws-sdk/client-cloudfront');
const iam = require('@aws-sdk/client-iam');
const AWSConfig = require('../aws-config.json');

const config = { region: AWSConfig.region };

// Generate command for API call that fetches Kinesis stream details 
...
(async function() {
  // Fetch deployed Kinesis stream's streamARN
  ...
  const createConfigCommand = generateCommand(streamARN, roleARN);
  let cloudFrontConfig = await cloudFrontClient.send(createConfigCommand);
