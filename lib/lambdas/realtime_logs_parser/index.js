...
function insertLogsIntoDB(logs) {
  ...
  const req = http.request(options, (res) => {
    ...
    res.on('end', () => {
      ...
      switch (true) {
        case (statusCode >= 500):
          throw new Error(`Something is wrong with the Database server. Response status code: ${statusCode}`);
        case (statusCode >= 400):
          throw new Error(`Something is wrong with your request or query. Response status code: ${statusCode}`);
        case (statusCode === 200):
          console.log(`Logs successfully inserted into the database`);
          break;
        default:
          console.log("response body:", responsePayload);
          throw new Error(`Whoops! Something Went wrong. Response status code: ${statusCode}`);
      }
    });
  });
